...
class GlassObject(OpticObject):
    ...
    def transfer_rays(self, bundle, n1=1.0, n2=1.0):
        ...
        def intersection1(x, ray_pos, ray_direc):
            ...
            return_val = new_ray[:, 0] - self.give_surface_1(new_ray[:, 1:3])-self.location
            return return_val

        def intersection2(x, ray_pos, ray_direc):
            new_ray = ray_pos + (np.atleast_2d(x).T)*ray_direc
            return_val = new_ray[:, 0] - self.give_surface_2(new_ray[:, 1:3])-self.location
            return return_val
        ...
        for i in range(0, ray_pos.shape[0], self.stepping):
            ...
            initial = (self.location - ray_pos[i:i+self.stepping, 0])/ray_direc[i:i+self.stepping, 0]
            result = optimize.root(intersection1, initial, args=(ray_pos[i:i+self.stepping], ray_direc[i:i+self.stepping]), jac=jacobian)
        ...
        mask = np.logical_and.reduce([r < self.clear_aperture/2, ~np.isnan(ray_direc[:, 0]), distance[:, 0] > 0])
        ...
        for i in range(0, ray_pos.shape[0], self.stepping):
            ...
            result = optimize.root(intersection2, initial, args=(ray_pos[i:i+self.stepping], ray_direc[i:i+self.stepping]), jac=jacobian)
            if not result.success:
                initial = 2* initial
                result = optimize.root(intersection2, initial, args=(ray_pos[i:i+self.stepping], ray_direc[i:i+self.stepping]), jac=jacobian)
                result.success = True
...
class Lens(GlassObject):
    """docstring for Lens"""
    def __init__(self, parameters, material, aperture, clear_aperture, thickness, location, surface_type='asphere'):
        super(Lens, self).__init__(material, aperture, clear_aperture, thickness, location)
...
class Detector(OpticObject):
    ...
    def transfer_rays(self, bundle, n1=1.0, n2=1.0):
        ...
        for i in range(0, ray_pos.shape[0], self.stepping):
            ...
            initial = (self.location - ray_pos[i:i+self.stepping, 0])/ray_direc[i:i+self.stepping, 0]
            result = optimize.root(intersection, initial, args=(ray_pos[i:i+self.stepping], ray_direc[i:i+self.stepping]), jac=jacobian)
            try:
                distance = np.vstack([distance, np.atleast_2d(result.x).T])
            except (ValueError, UnboundLocalError):
                distance = np.atleast_2d(result.x).T
        ...
        slit = self.slit
        if not slit > 0:
            mask = np.logical_and.reduce([r <= self.aperture/2, ~np.isnan(ray_direc[:, 0]), distance[:, 0] >= 0])
        else:
            z = ray_pos[:, 2]
            mask = np.logical_and.reduce([r <= self.aperture/2, np.abs(z)<slit/2, ~np.isnan(ray_direc[:, 0]), distance[:, 0] >= 0])
