...
class Bundle(object):
    ...
    def intersect(self, other_bundle):
        _, index1, index2 = np.intersect1d(
            self.indices, other_bundle.indices, return_indices=True
        )
...
class RaySource(object):
    ...
    def __init__(
        self, N, positions, distributed, laserbeamsize, wavelength, direction="x"
    ):
        ...
        if not self.distributed:
            for p in self.positions:
                if self.laserbeamsize > 0:
                    pos = (
                        self.laserbeamsize * np.random.rand(self.N, 2)
                        - self.laserbeamsize / 2
                    )
                    while np.any(((pos ** 2).sum(axis=1)) ** 0.5 > self.laserbeamsize):
                        not_ok = ((pos ** 2).sum(axis=1)) ** 0.5 > self.laserbeamsize
                        pos[not_ok] = (
                            self.laserbeamsize * np.random.rand(not_ok.sum(), 2)
                            - self.laserbeamsize / 2
                        )
                else:
